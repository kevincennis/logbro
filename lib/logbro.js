...
exports.setFormat = format => {
  if (!Object.keys(formats).includes(format)) {
    throw new Error(`Invalid Logbro format: ${format}`);
  }

  process.env.LOGBRO_FORMAT = format;
};
...
exports.setFormat(process.env.LOGBRO_FORMAT || 'pretty');
...
// format log messages
function formatPretty( type, args ) {
  var now = new Date().toISOString(),
    tmpl = '[%s] %s: %s\n',
    msg;

  msg = args[ 0 ] instanceof Error ? ( args[ 0 ].stack || args[ 0 ] ) :
    util.format.apply( util, args );

  return util.format( tmpl, now, type, msg );
}

function formatJson( type, args ) {
  const timestamp = new Date().toISOString();
  const message = args[ 0 ] instanceof Error
    ? ( args[ 0 ].stack || args[ 0 ] )
    : util.format.apply( util, args );

  const log = { level: type, timestamp, message };
...
function log( type, level ) {
  ...
  if ( level >= loglevel ) {
    ...
    const format = process.env.LOGBRO_FORMAT || 'pretty';
    msg = formats[format]( type, args );
