...
class Logbro extends EventEmitter {
  ...
  /**
   * Construct an instance of Logbro
   *
   * @param {Options} opts
   */
  constructor( opts = {} ) {
  ...
  set level( _ ) {
    const msg = 'Do not set the level of Logbro instance. ' +
                'Instead, set the Logbro level globally.';
    throw new Error( msg );
  }

  /**
   * Log the level and message to the appropriate stream
   *
   ...
   * @param {Object} [mergingObject] - see log()
